...
function normalizeRandomLootPoolRollCount(
  ...
  return Math.max(0, Math.floor(rollCount));
}
...
function rollRandomLootPool(input: {
  ...
  const rollCount = normalizeRandomLootPoolRollCount(input.pool.rollCount);
